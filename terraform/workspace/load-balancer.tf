resource "aws_lb_listener_rule" "microservice-listener-rule" {
  action {
    target_group_arn = aws_lb_target_group.microservice-target-group.arn
    type             = "forward"
  }
  condition {
    path_pattern {
      values = ["/${var.name}", "/${var.name}/*"]
    }
  }

  # Block cidrs directly
  # dynamic "condition" {
  #   for_each = var.allowed_ip_cidrs == null ? [] : [1]
  #   content {
  #     source_ip {
  #       values = var.allowed_ip_cidrs
  #     }
  #   }
  # }

  listener_arn = data.terraform_remote_state.core-services.outputs.alb_private_listener_https_arn
  priority     = var.alb_rule_priority
}

resource "aws_lb_target_group" "microservice-target-group" {
  name_prefix = substr(var.name, 0, 6)

  deregistration_delay = 30
  health_check {
    healthy_threshold   = 2
    interval            = 10
    path                = var.health_url
    protocol            = "HTTP"
    timeout             = 5
    unhealthy_threshold = 5
  }
  port        = var.application_port
  protocol    = "HTTP"
  target_type = "ip"
  vpc_id      = data.terraform_remote_state.core-services.outputs.vpc_main_id
}
